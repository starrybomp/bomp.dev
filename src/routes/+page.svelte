<script lang="ts">
  ...
  isChannelLive('starrybomp').then((meow) => { streaming = meow });

  setInterval(async () => {
    streaming = await isChannelLive('starrybomp')
  }, 15000)
</script>

{#if streaming == true}
  <div class="md:w-1/2 lg:w-1/3 aspect-video ml-4 mr-4 mt-6 mb-4 justify-items-center mx-auto flex-col">
    <iframe
      title="starrybomp's stream"
      src="https://player.twitch.tv/?channel=starrybomp&parent={hostName}"
      height="100%"
      width="100%"
...
{/if}
<div class="bg-zinc-900 md:w-1/2 lg:w-1/3 mt-6 mb-4 mr-4 ml-4 pl-8 pr-8 pt-8 pb-10 rounded-lg shadow justify-items-center mx-auto flex-col">
  <h1 class="text-4xl text-slate-100 font-bold rounded-lg"><img alt="fun little icon" class="inline mb-1 rounded-lg" src={image} height="48" width="48"/>&nbsp;starrybomp <h2 class="text-2xl text-slate-300 rounded-lg font-normal inline break-point">he/they</h2></h1>
